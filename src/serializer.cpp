...
Serializer::Serializer(const std::string& filename, bool write_) :
    ...
    mFile.open(filename, write_ ? (std::ios::out | std::ios::trunc | std::ios::binary) : (std::ios::in | std::ios::binary));
    if (!mFile.is_open())
...
bool Serializer::get_base(const std::string& name,
    ...
    if (mWrite)
        throw std::runtime_error("\"" + mFilename + "\": not open for reading!");

    std::string fullName = mPrefixStack.back() + name;

    auto it = mTOC.find(fullName);
    if (it == mTOC.end()) {
        std::string message = "\"" + mFilename + "\": unable to find field named \"" + fullName + "\"!";
        if (!mCompatibility)
    ...
    if (record.first != type_id)
        throw std::runtime_error(
            "\"" + mFilename + "\": field named \"" + fullName + "\" has an incompatible type (expected \"" + type_id + "\", got \"" + record.first + "\")!");
...
void Serializer::set_base(const std::string& name,
    ...
    auto it = mTOC.find(fullName);
    if (it != mTOC.end())
        throw std::runtime_error("\"" + mFilename + "\": field named \"" + fullName + "\" already exists!");
...
void Serializer::read(void* p, size_t size) {
    ...
    if (!mFile.good())
        throw std::runtime_error("\"" + mFilename + "\": I/O error while attempting to read " + std::to_string(size) + " bytes.");
}
...
void Serializer::write(const void* p, size_t size) {
    ...
    if (!mFile.good())
        throw std::runtime_error(
            "\"" + mFilename + "\": I/O error while attempting to write " + std::to_string(size) + " bytes.");
}
...
void Serializer::seek(size_t pos) {
    ...
    if (!mFile.good())
        throw std::runtime_error(
            "\"" + mFilename + "\": I/O error while attempting to seek to offset " + std::to_string(pos) + ".");
}

...
int ImagePanel::indexForPosition(const Vector2i& p) const {
    Vector2f pp = ((p - mPos).cast<float>() - Vector2f::Constant(mMargin)) / (float)(mThumbSize + mSpacing);
    ...
    overImage &= ((gridPos.array() >= 0).all() && (gridPos.array() < grid.array()).all());
...
void ImagePanel::draw(NVGcontext* ctx) {
    ...
    for (size_t i = 0; i < mImages.size(); ++i) {
        Vector2i p = mPos + Vector2i::Constant(mMargin) + Vector2i((int)i % grid.x(), (int)i / grid.x()) * (mThumbSize + mSpacing);
        int imgw, imgh;
